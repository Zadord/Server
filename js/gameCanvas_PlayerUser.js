...
class PlayerUserType extends PlayerType {
    constructor(akvOptionsIn) {
        ...
        this.dWalkSpeed = 200.0;
        this.dJumpSpeed = 340.0;
        this.dAccelY = 220.0;
        this.dSpeedX = 0;
        this.dSpeedY = 0;
    ...
    update(adElapsedTime, aTiles) {
        ...
        this.dSpeedX = 0;
        if (Keyboard.isJump() && PlayerUserType.jumpaudio == 0) {
            PlayerUserType.jumpaudio = 1;
            ManagerAudio.play("footstepsed");
        }
        else if (!Keyboard.isJump() && PlayerUserType.jumpaudio == 1) {
            PlayerUserType.jumpaudio = 0;
        }
        if (Keyboard.isKick() && PlayerUserType.kickaudio == 0) {
            PlayerUserType.kickaudio = 1;
            ManagerAudio.play("punch-kick");
        }
        else if (!Keyboard.isKick() && PlayerUserType.kickaudio == 1) {
            PlayerUserType.kickaudio = 0;
        }
        if (Keyboard.isJump()) {
            aePlayerState = JUMP;
        }
        ...
        const avTilesColliding = aTiles.getCollidingTiles(akvBoundingBox_new), anTiles = avTilesColliding.length;
        if (0 >= anTiles) {
